...
2. Read the file targets and proof command from the dispatch prompt (provided by the driver from the execution contract). Stay within the declared target files. If you need to modify a file not listed in targets, note it in your return.
...
8. **Section-local proof**: run the proof command provided in the dispatch prompt. This must pass before proceeding.
...
1. Run section-local proof (the proof command from the dispatch prompt). Full suite is the driver's responsibility at batch/global proof.
