...
3. Run tests against oracle/reference
...
- Oracle comparison: run same input against reference system, compare output
...
3. Commit with message: `feat(<project>): oracle-verify section X.Y — <name>`
...
| Test pattern | Table-driven Go tests with `startOracle()` helper |
| Oracle command | `docker exec mysql-8 mysql -e "SHOW CREATE TABLE ..."` |
